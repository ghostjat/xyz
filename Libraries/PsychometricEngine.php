...
class PsychometricEngine {
    ...
    public function calculateScore(string $moduleName, array $answers) {
        switch ($moduleName) {
            ...
            case 'aptitude':
                ...
                break;
            case 'vark':
                $result = $this->processVark($answers);
                break;
            case 'motivators':
                $result = $this->processMotivators($answers);
                break;
            default:
                break;
        }
    ...
    public function processAptitude(array $answers) {
        ...
        // Tracking metrics for Cognitive Agility
        $totalTime = 0;
        ...
        foreach ($answers as $a) {
            ...
            // Extract the seconds taken from the controller
            $timeTaken = isset($a['time_taken']) ? (int)$a['time_taken'] : 60;
            $totalTime += $timeTaken;

            // CRITICAL FIX 1: Use $rawScores to check if initialized, NOT $cats
            // This stops the engine from resetting your score to 0 on every question!
            if (!isset($rawScores[$k])) { 
                $rawScores[$k] = 0; 
                $maxPoints[$k] = 0; 
            }
            
            // --- COGNITIVE SPEED BONUS ALGORITHM ---
            $questionScore = $val;
            
            // If they got the answer right ($val > 0) AND answered fast (under 30s)
            // They get a speed bonus (Up to 20% extra points for this specific question)
            if ($val > 0 && $timeTaken < 30) {
                $speedEfficiency = (30 - $timeTaken) / 30; // Closer to 0 seconds = higher efficiency
                $bonusMultiplier = $speedEfficiency * 0.20; // Max 20% bonus
                $questionScore += ($val * $bonusMultiplier);
            }
            
            $rawScores[$k] += $questionScore;
            $maxPoints[$k] += $max;
        }
        ...
        foreach ($normalized as $cat => $score) {
            // CRITICAL FIX 2: Map DB names ("Numerical Ability") to your internal norms ("numerical")
            // This extracts the first word and makes it lowercase so it finds the correct baseline.
        ...
        // ═══════════════════════════════════════════════════════════════════
        // STEP 4: COMPOSITE IQ ESTIMATION (Smart Keyword Matching)
        // ═══════════════════════════════════════════════════════════════════
        $weightedTScore = 0;
        $totalWeight = 0;
        ...
        // Identify the processing speed behavioral trait
        $processingBand = 'Average Processing Speed';
        if ($avgTime <= 25 && $finalCompositeT >= 55) {
            $processingBand = 'High Cognitive Agility (Fast & Accurate)';
            $iqEstimate += 5; // Strategic IQ bump for gifted speed processing
        } elseif ($avgTime <= 25 && $finalCompositeT < 45) {
            $processingBand = 'Impulsive (Fast but Inaccurate)';
            $iqEstimate -= 5; // IQ penalty for guessing
        } elseif ($avgTime >= 45) {
            $processingBand = 'Deliberate / Methodical (Slower Processing)';
        }
        ...
        return [
            ...
            'iq_projection' => [
                ...
                'cognitive_agility' => $processingBand,
    ...
    public function processVark(array $answers) {
        // FIXED: Matched exactly to the SQL database strings
        ...
        // Normalize to 0-100 Scale (Absolute percentage of max possible score)
        $normalized = [];
        ...
        foreach ($normalized as $cat => $score) {
            // Converts to lowercase for array key matching in getNorm()
            $norm = $this->getNorm('VARK', strtolower($cat));
            $z = ($score - $norm['mean']) / $norm['std_dev'];
            
            $t = 50 + (10 * $z);
            $standardized[$cat] = round(max(20, min(80, $t)), 1);
            $percentiles[$cat] = $this->zToPercentile($z);
            $stanines[$cat] = $this->calculateStanine($z); // Make sure this calls the right method!
        }
        ...
        $secondary = $sortedKeys[1] ?? $primary; // Fallback
        ...
        if ($diff <= 4) {
            // Multimodal (e.g., "Visual + Kinesthetic")
        ...
        return [
            // This 'scores' key is what final_pharos_report.php reads for the bar charts!
            'scores' => $normalized, 
            'standardized' => [
                't_scores' => $standardized,
                'percentiles' => $percentiles,
    ...
    /**
     * ═══════════════════════════════════════════════════════════════════════
     * 7. CAREER MOTIVATORS (WORK VALUES) ALGORITHM
     * ═══════════════════════════════════════════════════════════════════════
     * * METHODOLOGY:
     * - Calculates Absolute Percentages based on maximum possible Likert scores.
     * - Converts to standardized T-Scores for accurate psychological profiling.
     * - Identifies the primary intrinsic driver.
     */
    public function processMotivators(array $answers) {
        // EXACT DB Strings matching the SQL insert
        $categories = [
        ...
        // Normalize to 0-100 Scale (Absolute percentage)
        $normalized = [];
        foreach ($rawScores as $cat => $sum) {
            // e.g., 3 questions * 5 max points = 15 possible points
            $maxPossible = $counts[$cat] * 5; 
            $normalized[$cat] = $maxPossible > 0 ? round(($sum / $maxPossible) * 100, 1) : 0;
        }
        ...
        foreach ($normalized as $cat => $score) {
            // Converts to lowercase to match the norms array
            $norm = $this->getNorm('MOTIVATORS', strtolower($cat));
            $z = ($score - $norm['mean']) / $norm['std_dev'];
            
            $t = 50 + (10 * $z);
            // Cap T-Scores between 20 and 80 to prevent clinical outliers
            $standardized[$cat] = round(max(20, min(80, $t)), 1);
            $percentiles[$cat] = $this->zToPercentile($z);
            $stanines[$cat] = $this->calculateStanine($z);
        }
        ...
        // Sort T-Scores highest to lowest to find their true primary driver
        arsort($standardized);
        $sortedKeys = array_keys($standardized);
        ...
        return [
            // $normalized is passed as 'scores' because your PDF report bar charts
            // use 0-100% absolute percentages, not T-Scores!
            'scores' => $normalized, 
            'standardized' => [
                't_scores' => $standardized,
                'percentiles' => $percentiles,
    ...
    private function loadNormativeData(): void {
        ...
        $this->norms = [
            ...
            'GARDNER' => [
                ...
                'Numerical' => ['mean' => 3.3, 'std_dev' => 0.85], // Added Likert norm
                'Logical'   => ['mean' => 3.3, 'std_dev' => 0.85], // Renamed to match DB
                'Spatial'   => ['mean' => 3.0, 'std_dev' => 0.91], // Renamed to match DB
                'Auditory'          => ['mean' => 3.1, 'std_dev' => 0.89], // Added Likert norm
                'Kinesthetic'       => ['mean' => 2.9, 'std_dev' => 0.93], // Renamed to match DB
            ...
            'APTITUDE' => [
                ...
                'spatial'    => ['mean' => 60, 'std_dev' => 16.0], // Often harder for general pop
                'mechanical' => ['mean' => 58, 'std_dev' => 15.0], // Requires specific specialized logic
                'accuracy'   => ['mean' => 75, 'std_dev' => 12.0], // Perceptual speed is usually higher
            ...
            'VARK' => [
                ...
                'uditory'       => ['mean' => 52, 'std_dev' => 13],
                'read & write'   => ['mean' => 48, 'std_dev' => 14], // Often lower in general pop
