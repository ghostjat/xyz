...
private function extractScore(array $scoresObj, string $key): float
{
    ...
    /**
     * 1. Get all unique clusters and their career counts.
     * Aliases 'cluster_name' to 'career_category' to fix the Undefined Key error in the view.
     */
    public function getCareerClusters() {
        
        $db = \Config\Database::connect();
        
        // Use AS to provide the key the View (cluster.php) is expecting
        $clusters = $db->table('career_clusters')
                       ->select('id, cluster_name AS career_category') 
                       ->where('is_active', 1)
                       ->orderBy('cluster_name', 'ASC')
                       ->get()->getResultArray();

        // Calculate counts from the career_paths table for the UI badges
        foreach ($clusters as &$cluster) {
            $cluster['total_careers'] = $db->table('career_paths')
        ...
        // This now passes 'career_category' to APPPATH/Views/dashboard/cca/cluster.php
        return view('dashboard/cca/cluster', $data);
    }
    
    /**
     * 2. Get all careers (paths) inside a specific cluster.
     * Linked via cluster_id to maintain data integrity.
     */
    ...
    // 1. Try to get numeric ID (best practice)
    $clusterId = $this->request->getPost('id', FILTER_SANITIZE_NUMBER_INT);
    
    // 2. Fallback: If ID is missing, find the ID using the Category Name string
    if (empty($clusterId)) {
        $categoryName = $this->request->getPost('category');
        $clusterRecord = $db->table('career_clusters')
                            ->select('id')
                            ->where('cluster_name', $categoryName)
                            ->get()->getRowArray();
        $clusterId = $clusterRecord['id'] ?? null;
    }

    // 3. Final Check: If we still have no ID, return empty early
    if (!$clusterId) {
        return view('dashboard/cca/clustercareers', ['careers' => [], 'category_name' => 'Unknown']);
    }

    // 4. Fetch the actual career paths linked to this ID
    $careers = $db->table('career_paths')
                  ->select('id, title as career_title, roles as short_description')
    ...
    $data = [
        'careers' => $careers, 
        'category_name' => $this->request->getPost('category') ?? 'Selected Category'
    ];
    ...
    /**
     * 3. Get exhaustive details for a specific Career Path.
     * Decodes the granular JSON requirements for the Modal view.
     */
    public function getCareerDetails() {
        ...
        // Fetch from career_paths joined with cluster name for context
        $career = $db->table('career_paths')
                     ->select('career_paths.*, career_clusters.cluster_name as career_category')
                     ->join('career_clusters', 'career_clusters.id = career_paths.cluster_id')
                     ->where('career_paths.id', $id)
                     ->get()->getRowArray();

        if (!$career) {
            return "<tr><td class='text-danger text-center'>Career path not found.</td></tr>";
        }

        // Decode the specific JSON columns used in the latest database schema
        $jsonFields = [
            'riasec', 
            'mbti', 
            'aptitude_weights', 
            'gardner_requirements', 
            'eq_requirements'
        ];
        ...
        // Add alias for the view to prevent "Undefined Index" if your view uses 'career_title'
        $career['career_title'] = $career['title'];
        $career['short_description'] = $career['roles'];
